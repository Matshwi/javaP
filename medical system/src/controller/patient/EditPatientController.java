...
public class EditPatientController implements Initializable {
    ...
    private void showImage(Patient patient) {
        if (patient.getThumbnail() == null) {
            imageViewPatientImage.setImage(new Image(getClass().getResource("/image/avater.jpg").toExternalForm()));
        } else {
            File nFile = new File(path + "\\Documents\\DrAssistant\\" + patient.getThumbnail());
            try {
